function(allstates,event,...)
    ...
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        if subevent == "SPELL_CAST_SUCCESS" then
            ...
            print("spell success id = "..tostring(spellId))
        ...
        if unit then
            ...
            if sourceGUID then
                ...
                if aura_env.spellIDs[tostring(spellId)] and aura_env.npcIDs[npcID] then
                    ...
                    for i,v in pairs(aura_env.inspected) do
                        if(v.priority < memberInfo.priority) and (v.expirationTime < (GetTime() + castTime - 0.5)) then
                            memberInfo = v
                        elseif v.priority == memberInfo.priority then
                            if memberInfo.unit == 0 or UnitGUID(v.unit) > UnitGUID(memberInfo.unit) then
                                memberInfo = v
                            end
                        end
                    end
                    ...
                    if spellId == 227628 then
                        ...
                        if debuffStack and debuffStack < aura_env.config.debuffMaxStack then
                            needInterrupt = 0
                        end
                    end
                    ...
                    if (spellId == 227615) then
                        ...
                        if state and (aura_env.config.purgatoryIntr == false) then
                            local name = AuraUtil.FindAuraByName(spellName, unit.."target","HARMFUL")
                            if (GetTime() > state.dangerousTime) and (name == nil) then
                                needInterrupt = 0
                            end
                        end
                    end
